...
namespace WTCJ.Controllers
{
    public class DepartmentController : Controller
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult ExcelExport(string DepartmentName)
        {
            string where = "";
            if (!string.IsNullOrEmpty(DepartmentName) && CommonHelper.filterSql(DepartmentName))
            {
            ...
            string filePathTemplate = Server.MapPath("~/") + @"\Excel\Template\配置信息\部门管理模板.xlsx";
            new NOPI_ExcelHelp().OutputExcel(dt, filePathTemplate, this.HttpContext
                , "部门管理信息" + DateTime.Now.ToString("yyyyMMddHHmmss"));
            return null;
        }
        /// <summary>
        ...
        public ActionResult ExcelImport()
        {
            ...
            //查询和模板页列是否相同
            DataTable dtTemplate = NOPI_ExcelHelp.ImportDataTableFromExcel2007(Server.MapPath("~/") + @"\Excel\Template\配置信息\部门管理模板.xlsx");
            json = new Excel.DepartmentExcel().Import(dt, dtTemplate);
            return Json(json);
        }
    }
}
